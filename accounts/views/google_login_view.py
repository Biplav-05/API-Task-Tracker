...
from accounts.serializers.user_profile_serializer import UserDetailSerailizer
from accounts.serializers.google_login_serializer import GoogleLoginSerializer
from accounts.services.google_service import get_google_user, validate_google_access_token
...
class UserGoogleLoginView(APIView):
    ...
    def post(self, request):
        ...
        user_instance, created =CustomUserAccount.objects.get_or_create(
        ...
        data = {"data": {"access_token": access_token, "refresh_token": refresh_token}}
...
class RefreshGenerationView(APIView):
    authentication_classes = []
    permission_classes = [AllowAny]
    ...
    def post(request):    
        ...
        user = CustomUserAccount.objects.filter(id=payload.get("user_id"), is_active=True).first()
